...
object TcbCrc32 {
    ...
    fun generate(data: ByteArray): ByteArray {
        val rem = data.size % 4
        val len = if (rem != 0) data.size + 4 - rem else data.size
        var crc = 0xFFFFFFFFL
        for (i in 0 until len) {
            val b = if (i < data.size) (data[i].toLong() and 0xFF) else 0xFFL
            crc = ((crc shl 8) xor TABLE[((crc shr 24) xor b).toInt() and 0xFF]) and 0xFFFFFFFFL
        }
        return byteArrayOf(
